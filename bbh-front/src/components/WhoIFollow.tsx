...
export default function WhoIFollow() {
    ...
    async function fetchFolloweeList() {
        try {
            const response:ResponseData<FollowsDTO> = await httpService.empty<FollowsDTO>('/whoIFollow',"GET");
            if (response.success===false) {
                setError(response.errorMsg)
                console.log(response.errorMsg)
                return
            }
            const sortedList:FollowDTO[] = response.data.follows.sort((a, b) => a.target.nickname.localeCompare(b.target.nickname,'zh-CN'))
    ...
    const renderFolloweeItem = (followee: FollowDTOWithStatus) => (
        <li className="list-row" key={followee.id}>
            ...
            <div>
                ...
                <div className="text-xs font-semibold opacity-60">@{followee.target.username}</div>
