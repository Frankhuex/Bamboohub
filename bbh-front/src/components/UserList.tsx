...
export default function UserList({userDTOs, setUserDTOs}: UserListProps) {
    ...
    // Don't use `const [loading, setLoading] = useState(false)` here,
    // Bebause it will cause re-render every time `loading` changes,
    ...
    const renderFolloweeItem = (user: UserDTOWithFollow) => {
        const followButton = (toggledId === user.id) 
            ...
            : (<button onClick={() => toggleFollow(user.id)} className="btn w-25">
