...
interface UserListProps {
    ...
    setUserDTOs: (newUserDTOs: UserDTOWithFollow[])=>void;
}
...
export default function UserList({userDTOs, setUserDTOs}: UserListProps) {
    ...
    const renderFolloweeItem = (user: UserDTOWithFollow) => (
        <li className="list-row" key={user.id}>
            ...
            <button onClick={() => toggleFollow(user.id)} className="btn">
                <svg xmlns="http://www.w3.org/2000/svg" fill={user.followed===true ? 'black' : "none"} viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" className="size-[1.2em]"><path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" /></svg>
    ...
    const toggleFollow = async (targetId: number) => {
        ...
        try {
            ...
            if (response.success) {
                const newUserDTOs: UserDTOWithFollow[] = userDTOs?.map(user => 
                    user.id === targetId 
                    ? {...user, 
                ...
                console.log(newUserDTOs)
                setUserDTOs(newUserDTOs)
            }
        } catch (error) {
